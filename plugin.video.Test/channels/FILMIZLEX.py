...
def main():
        url='http://www.filmizlexx.com/ads/'
        ...
        match=re.compile('<li class="maincat cat-item-.*?"><a href="(.*?)" title=".*?">(.*?)</a>  </li>').findall(link)
...
def Yeni(url):
        ...
        page=re.compile('class="current">.*?</span><a href="(.*?)" class="page" title=".*?">(.*?)</a>').findall(link)
