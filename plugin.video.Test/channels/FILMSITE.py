...
def main():
        ...
        araclar.addDir(fileName,'[COLOR blue][B]>> [/B][/COLOR][COLOR lightblue][B]Yeni Eklenen Filmler [/B][/COLOR]', "Yeni(url)",url,"http://www.filmsiteleri.net/wp-content/themes/baymovie/images/logo.png" )
                ##### KATEGORILERI OKU EKLE ##########################
        
        link=araclar.get_url(url)
        match=re.compile('class="cat-item cat-item-.*?"><a  href="(.*?)" title=".*?">(.*?)</a>\n').findall(link)
...
def Yeni(url):
        ...
        page=re.compile('<a  href="(.*?)" class="page smaller">(.*?)</a>').findall(link)
